--[[ 
    SQUADRIM PRESTIGE v18.1 | FULLY FIXED
...
local function CreateHUD()
    ...
    RunService.RenderStepped:Connect(function()
        ...
        Label.Text = string.format("| t.me/squadrim1 | PRESTIGE | FREE | [%d FPS] |", avgFps)
    end)
end
...
local function ClearESP()
    for plr, obj in pairs(Storage) do
        pcall(function()
            ...
            if obj.Skel then
                for _, line in pairs(obj.Skel) do line:Remove() end
            end
        end)
    end
    Storage = {}
            ...
            if onScreen then
                ...
                if _G.Settings.Visuals.Skeleton then
                    ...
                    for i, points in ipairs(skelPoints) do
                        ...
                        if on1 and on2 then
                            if not s.Skel[i] then
                                s.Skel[i] = Drawing.new("Line")
                                s.Skel[i].Thickness = 1
                                s.Skel[i].Color = Color3.new(1,1,1)
                            end
                            s.Skel[i].Visible = true
                            s.Skel[i].From = Vector2.new(p1.X, p1.Y)
                            s.Skel[i].To = Vector2.new(p2.X, p2.Y)
                        elseif s.Skel[i] then
                            s.Skel[i].Visible = false
                        end
                    end
                elseif s.Skel then
                    for _, line in pairs(s.Skel) do pcall(function() line.Visible = false end) end
                end
            else
                if Storage[plr] then
                    pcall(function() 
                        ...
                        if Storage[plr].Skel then
                            for _, line in pairs(Storage[plr].Skel) do line.Visible = false end
                        end
                    end)
                end
            end
...
local function CreateMenu()
    ...
    MenuFrame.Position = UDim2.new(0.5, -275, 0.5, -225)
    ...
    Title.Text = "SQUADRIM PRESTIGE v18.1"
    Title.TextColor3 = Color3.fromRGB(200, 50, 50)
    Title.TextSize = 18
    Title.Font = Enum.Font.SourceSansBold
